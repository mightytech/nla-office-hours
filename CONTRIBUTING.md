...
disagreement with a design choice, a place where a conversation went somewhere
unexpected.

The easiest path: just talk to the AI about it. The `/friction-log` and
`/write-letter` skills do the heavy lifting. You describe what happened,
the AI asks clarifying questions, and the skill produces structured feedback with
your observations, reasoning, and recommendations in plain English. Your job is to
notice something and have a conversation about it. The skill handles the rest.
...
**More useful:** "I asked about how NLAs handle non-determinism and the answer
only referenced the probe report — but the framework's values files in several
projects show this in practice. The connection between the conceptual argument
and the actual implementation was there, but the NLA didn't make it."

**Less useful:** "The NLA should be better at connecting framework source to
article concepts."
...
observations. This works well for focused feedback on a specific behavior,
answer quality, or design choice.

**Structured letters** (via `/write-letter`) are the richer path, especially for
substantial feedback that spans multiple observations. A letter submitted as a
GitHub Issue gets triaged by the project's AI — each item evaluated against the
project's values, goals, and design philosophy, with a proposed verdict (accept,
adapt, defer, or decline) and reasoning for each. The maintainer reviews the
verdicts and decides.
...
contributions come from people with fresh perspectives on the ideas.

If you asked about NLA persistence patterns and the answer felt shallow — that's
a contribution. If you were exploring for the first time and the NLA's answers felt
too abstract when you wanted concrete examples — that's a contribution. If you
noticed the NLA handled a skeptical question defensively instead of generously —
that's a contribution.

Your experience of the conversation, described honestly and specifically, is exactly
the kind of feedback that makes this NLA better. You don't need to know how to fix
what you found. You just need to describe what happened and what you expected instead.
