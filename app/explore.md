...
**The source code** — The NLA Framework (`packages/nla-framework/`), this NLA's own files (`app/`, etc.), installed packages, and example NLAs in `examples/` (copydesk, duet, nla-claude-code — concrete working NLAs across distinct shapes: editorial workflow, creative composition, dev tooling). This is living evidence — how things actually work, details the articles couldn't fit, connections between pieces that only show up in the implementation. When someone asks how something works, or wants to see a real example of an NLA other than the framework, the code grounds the answer in what's built. The examples are particularly valuable when visitors ask comparative questions or want to see an NLA they've heard of in action.
